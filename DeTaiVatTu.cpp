...
void ThemVatTu(treeVT &root, VatTu vt){
    if(root == NULL){
        root = CreateNodeVT(vt);
        return;
    }
    int cmp = strcmp(vt.MAVT, root -> vt.MAVT);
    if (cmp ==0){
        std::cout <<"Ma vat tu da ton tai. Khong the them."<<std::endl;
        return;
    }else if(cmp<0){
        ThemVatTu(root->left, vt);
    }else{
        ThemVatTu(root->right, vt);
    }
}
